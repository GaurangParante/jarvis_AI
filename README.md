...
Say "generate an image of..." and Jarvis creates it locally by default, so it works without an image API key. If you set `POLLINATIONS_API_KEY`, it can also use Pollinations as an online fallback.
...
POLLINATIONS_API_KEY=optional_image_key_here
...
TTS_RATE=+22%
LOCAL_IMAGE_MODEL_ID=hf-internal-testing/tiny-stable-diffusion-pipe
LOCAL_IMAGE_STEPS=8
...
| `POLLINATIONS_API_KEY` | No | — | Optional Pollinations key for online image fallback |
...
| `LOCAL_IMAGE_MODEL_ID` | No | `hf-internal-testing/tiny-stable-diffusion-pipe` | Local fallback image model |
| `LOCAL_IMAGE_STEPS` | No | `8` | Number of local diffusion steps |
| `LOCAL_IMAGE_SIZE` | No | `512` | Output image size for the local fallback |
...
- Local image generation is used by default and does not need an API key
- If you set `POLLINATIONS_API_KEY`, make sure it is valid and the server has internet access
- If you see `No module named 'PIL'`, run `pip install -r requirements.txt` inside the same virtual environment used by `python run.py`
...
| Local diffusion + optional Pollinations fallback | Image generation |
