...
_QUERY_EXTRACTION_PROMPT = (
    "You are a search query optimizer. Convert the user's message into clean, focused  "
    "web search query (max 10 words) Rules:\n"
    "- Remove filler words (you know, like, somthing , can you, tell me, search)\n"
    "- Add specifics: dates (today, 2026), event names, full names\n"
    "- For sports: include league name, team name, 'live score today'\n"
    "- For people: include full name + what user wants to know \n"
    "- Resolve referances (him, that, it) from conversation history"
...
class RealtimeGroqService(GroqService):
    ...
    def search_tavily(self,query:str,num_results:int = 5)->str:
        ...
        try:
            ...
            if not results and not ai_answer:
                logger.warning(f"No Tavily search result for query: {query}")
                return ("",None)
            ...
            parts = [f"===WEB SEARCH RESULT FOR: {query} === \n"]
            ...
            logger.info(
                "[TAVILY] %d result, AI answer: %s, formatted: %d chars (%.3fs)",
    ...
    def get_response(self, question:str, chat_history:Optional[List[tuple]] = None,key_start_index:int = 0) -> str:
        ...
        try:
            ...
            prompt,message = self._build_prompt_and_messages(
            ...
            response_content = self._invoke_llm(prompt,message,question,key_start_index=key_start_index)
    ...
    def stream_response(self, question:str, chat_history : Optional[List[tuple]] = None,key_start_index:int = 0) -> Iterator[Any]:
        try:
            ...
            search_query = self._extract_search_query(question,chat_history)
            logger.info("[REALTIME] Searching Tavily for: %s",search_query)
            yield {"_activity":{"event":"seaching_web","query":search_query,"message":f"Searching web for: {search_query}"}}
    ...
    def stream_response_with_prefetched(
        ...
        try:
            ...
            yield from self._stream_llm(prompt,messages,question,key_start_index=key_start_index)
            logger.info("[REALTIME] Stream complated (pre-fetched results)")

        except ALLGroqApisFailedError:
            raise

        except Exception as e:
            logger.error("Error in realtime steram_response_with_prefetched: %s",e,exc_info=True)
            raise
