...
class ChatService:
    ...
    def get_or_create_session(self,session_id: Optional[str] = None) ->str:
        ...
        if not session_id:
            ...
            logger.info("[TIMING] session_get_create: %.3fs (new)",time.perf_counter() - t0)
        ...
        if session_id in self.sessions:
            logger.info("[TIMING] session_get_create: %.3fs (memory)",time.perf_counter() - t0)
            return session_id

        if self.load_session_from_disk(session_id):
            logger.info("[TIMING] session_get_create: %.3fs (disk)",time.perf_counter() - t0)
            return session_id

        self.sessions[session_id] = []
        logger.info("[TIMING] session_get_create: %.3fs (new_id)",time.perf_counter() - t0)
        return session_id
    ...
    def process_realtime_message(self, session_id:str, user_message:str)->str:
        ...
        logger.info("[REALTIME] Session: %s | User: %.200",session_id[:12],user_message)
        self.add_message(session_id,"user",user_message)

        chat_history = self.format_history_for_llm(session_id,exclude_last=True)
        logger.info("[REALTIME] Hisory pairs sent to LLM: %d",len(chat_history))
    ...
    def process_message_stream(self,session_id:str,user_message:str)->Iterator[str]:
        ...
        chat_history = self.format_history_for_llm(session_id,exclude_last=True)
        logger.info("[GENERAL-STREAM] Hisory pairs sent to LLM: %d",len(chat_history))
        chunk_count = 0
        try:
    ...
    def process_realtime_message_stream(
        ...
        logger.info("[REALTIME-STREAM] Sessio: %s | User: %.200s",session_id[:12],user_message)

        self.add_message(session_id,"user",user_message)
        self.add_message(session_id,"asistant","")
        chat_history = self.format_history_for_llm(session_id,exclude_last=True)
