...
ALL_APIS_FAILED_MESSAGE = (
    "I'm unable to process your request at the moment. All API service are "
    "temporarily unavailable. Please try again in few minutes."
)
...
def _truncate_at_repetition(text:str)->bool:
...
class GroqService:
    ...
    def _stream_llm(
        ...
        for j in range(n):
            ...
            try:
                ...
                if i < n - 1:
                    logger.info(f"Falling back to next API key for stream...")
                    continue
                break
    ...
    def _build_prompt_and_messages(
        ...
        logger.info("[PROMPT] System message length: %d chars | Histoy pairs: %d | Question: %.100s",
